...
EmailTemplateRenderer.prototype.init = function(callback) {
	...
	function compileTemplates(dirs, next) {
			...
			fs.readdir(
				dirPath, function(err, files) {
					...
					function processFile(file, next) {
						...
						fs.readFile(
							filePath, options, function(err, contents) {
								if (err) {
									next(err);
									return;
								}
		...
		async.filter(
			dirs, isValidDirectory, function(results) {
				async.eachSeries(
					results, compileTemplate, function(err) {
	...
	this.init(
		function(err) {
			...
			function render(data, next) {
				...
				juice.juiceContent(
					html, options, function(err, html) {
						result[data.type] = html;
						next(err);
					});
			}
			...
			async.each(
				templateData, render, function(err) {
					callback(err, result);
				});
		});
};
